...
public class ElasticDragDismissFrameLayout extends FrameLayout {
    ...
    public interface ElasticDragDismissCallback {
         ...
         */
        void onDragDismissed(int orientation);
    ...
    public void onNestedScroll(View target, int dxConsumed, int dyConsumed,
        ...
        if (5 < Math.abs(dyUnconsumed)) {
            Log.d("wtf","offset "+ dyUnconsumed);
            dragScale(dyUnconsumed);
            preScroll = dyUnconsumed;
        }
    }
    ...
    public void onStopNestedScroll(View child) {
        if (Math.abs(totalDrag) >= dragDismissDistance) {
            if (draggingDown)
                dispatchDismissCallback(ElasticDragDismissCallback.DISMISS_DOWN);
            else
                dispatchDismissCallback(ElasticDragDismissCallback.DISMISS_UP);
    ...
    private void dragScale(int scroll) {
        ...
        //float dragFraction = (float) Math.log10(1 + (Math.abs(totalDrag) / getHeight()));
        // calculate the desired translation given the drag fraction
         float dragTo = dragFraction * dragDismissDistance * dragElacticity;

        //float dragTo = dragFraction * getHeight() * dragElacticity;
    ...
    private void dispatchDismissCallback(int oritention) {
        if (callbacks != null && !callbacks.isEmpty()) {
            for (ElasticDragDismissCallback callback : callbacks) {
                callback.onDragDismissed(oritention);
            }
        }
    }
